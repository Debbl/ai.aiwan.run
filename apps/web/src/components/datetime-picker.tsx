...
function format(date: Date, formatStr: string, options?: FormatOptions) {
...
export function DateTimePicker({
  ...
  const displayFormat = useMemo(() => {
    ...
    return format(
      displayValue,
      `yyyy, ${!hideTime ? 'MMM' : 'MMMM'} d${!hideTime ? (use12HourFormat ? ' hh:mm:ss a' : ' HH:mm:ss') : ''}`,
    )
    ...
    <Popover open={open} onOpenChange={setOpen} modal={modal}>
      <PopoverTrigger asChild>
        {renderTrigger ? (
          ...
          <div
            className={cn(
              'border-input bg-background flex h-9 w-full cursor-pointer items-center rounded-md border ps-3 pe-1 text-sm font-normal shadow-sm',
        ...
        <div className='flex items-center justify-between'>
          <div className='text-md ms-2 flex cursor-pointer items-center font-bold'>
            <div>
              ...
              <span className='ms-1' onClick={() => setMonthYearPicker(monthYearPicker === 'month' ? false : 'month')}>
                {format(month, 'MMMM')}
              </span>
            </div>
